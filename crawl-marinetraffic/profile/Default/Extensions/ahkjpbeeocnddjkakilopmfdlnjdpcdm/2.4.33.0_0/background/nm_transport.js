...
ns.NMSCaller = function NMSCaller()
{
    ...
    function ConnetToNativeMessaging(callbackSuccess, callbackError)
    {
        ...
        m_port = {};
        m_port.disconnectHandlers = [];
        m_port.onDisconnect = {};
        m_port.onDisconnect.addListener = fn => { m_port.disconnectHandlers.push(fn); };
        m_port.onDisconnect.removeListener = fn =>
            {
                const funcIndex = m_port.disconnectHandlers.indexOf(fn);
                if (funcIndex !== -1)
                    m_port.disconnectHandlers.splice(funcIndex, 1);
            };
        m_port.disconnect = () =>
            {
                for (const handler of m_port.disconnectHandlers)
                    handler();
            };
        m_port.postMessage = () => {};
        m_port.remote = {};
        ...
        m_port.remote.onMessage.addListener = fn =>
            {
                m_port.postMessage = msg => { fn(msg, m_port.remote); };
            };
        m_port.remote.onMessage.removeListener = () => { m_port.postMessage = () => {}; };

        m_port.remote.postMessage = () =>
            {
                if (!m_connected)
                {
                    m_connected = true;
                    m_port.remote.postMessage = msg => { ProcessMessage(msg); };
                    if (callbackSuccess)
                        callbackSuccess();
                }
            };
